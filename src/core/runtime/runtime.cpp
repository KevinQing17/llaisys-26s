...
Runtime::Runtime(llaisysDeviceType_t device_type, int device_id)
    ...
    // CUDA streams belong to the device that is current when they are created.
    // Select this runtime's device first so a lazily-created multi-GPU runtime
    // never inherits the previously active device's stream.
